...
interface Props{
    ...
    buttonTheme: "large" | "medium" | "small" | "varySmall";
    maxAvatars: number;
    className?: string;
}

const AvatarsStacked: React.FC<Props> = ({children, buttonTheme, className, maxAvatars}) => {
    const [isHidden, setIsHidden] = useState(true)
    const [translateY, setTranslateY] = useState("0")

    let themeClass: string;
    if(buttonTheme === "large"){
        themeClass = "py-2 px-3 text-lg rounded-2xl"
    }else if(buttonTheme === "medium"){
        themeClass = "py-1 px-3 rounded-xl"
    }else if(buttonTheme === "small"){
        themeClass = "px-0.5 py-1 text-sm rounded-lg"
    }else if(buttonTheme === "varySmall"){
    ...
    return(
        <div className={`relative flex max-w-max items-center ${className}`}>
            {children && children!.map((item: any, index: number) =>{
                ...
                return(
                    <>
                    <div className={`border-4 border-white rounded-full transform translate-y-28 ${avatarHiddenClass}`} style={{transform: "translatex(" + translateX + ")"}}>{item}</div>
